...
const paletteDefinitions = {
  ...
  nebula: {
    dark: { backgroundStart: "#0A0620", backgroundEnd: "#160B3B", panel: "#140A30", primary: "#EDE9FE", muted: "#9084BE", cyan: "#38BDF8", blue: "#6366F1", violet: "#C084FC", green: "#34D399", red: "#FB7185", scanBlend: "screen" },
    light: { backgroundStart: "#F5F3FF", backgroundEnd: "#EEF2FF", panel: "#FFFFFF", primary: "#2E1065", muted: "#6D5A9E", cyan: "#0EA5E9", blue: "#4F46E5", violet: "#9333EA", green: "#047857", red: "#DC2626", scanBlend: "multiply" }
  }
};
...
function buildGlitchLayer(layout, colors) {
  const visual = layout.visualPanel;
  const barX = visual.x + 4;
  const barWidth = visual.width - 8;

  const bars = [
    { y: visual.y + visual.height * 0.2, h: 4, dur: "3.2s", begin: "0.9s", color: colors.red, dx: 16 },
    { y: visual.y + visual.height * 0.46, h: 3, dur: "4.1s", begin: "2.1s", color: colors.cyan, dx: -12 },
    { y: visual.y + visual.height * 0.68, h: 5, dur: "3.7s", begin: "1.4s", color: colors.violet, dx: 20 },
    { y: visual.y + visual.height * 0.84, h: 3, dur: "4.6s", begin: "3.2s", color: colors.blue, dx: -18 }
  ];

  const barMarkup = bars.map((bar) => `
  <rect x="${barX}" y="${bar.y.toFixed(1)}" width="${barWidth}" height="${bar.h}" fill="${bar.color}" opacity="0">
    <animate attributeName="opacity" values="0;0;0.82;0.08;0.65;0;0" keyTimes="0;0.85;0.88;0.91;0.93;0.96;1" dur="${bar.dur}" begin="${bar.begin}" repeatCount="indefinite"/>
    <animateTransform attributeName="transform" type="translate" values="0 0;0 0;${bar.dx} 0;${-bar.dx / 2} 0;0 0" keyTimes="0;0.85;0.89;0.93;1" dur="${bar.dur}" begin="${bar.begin}" repeatCount="indefinite"/>
  </rect>`).join("");

  const rgbSplit = `
  <g opacity="0">
    <animate attributeName="opacity" values="0;0;0.5;0;0.35;0;0" keyTimes="0;0.85;0.87;0.9;0.92;0.95;1" dur="5.3s" begin="1.8s" repeatCount="indefinite"/>
    <rect x="${visual.x}" y="${visual.y}" width="${visual.width}" height="${visual.height}" fill="none" stroke="${colors.violet}" stroke-width="1.5" transform="translate(-3 0)" rx="${visual.radius}"/>
    <rect x="${visual.x}" y="${visual.y}" width="${visual.width}" height="${visual.height}" fill="none" stroke="${colors.cyan}" stroke-width="1.5" transform="translate(3 0)" rx="${visual.radius}"/>
  </g>`;

  const errorFlash = `
  <text x="${visual.x + visual.width - 14}" y="${visual.y + 22}" text-anchor="end" class="glitch-error" fill="${colors.red}" opacity="0">SIGNAL_ERR::0x2F
    <animate attributeName="opacity" values="0;0;1;0;1;0;0" keyTimes="0;0.55;0.565;0.6;0.615;0.65;1" dur="6.8s" begin="2.6s" repeatCount="indefinite"/>
  </text>`;

  return `<g clip-path="url(#glitch-clip)" aria-hidden="true">${barMarkup}${rgbSplit}${errorFlash}</g>`;
}
...
function createHeroSvg(config, colors, size, portrait) {
  ...
  const glitchLayer = buildGlitchLayer(layout, colors);
  const isDesktop = size === "desktop";
...
<defs>
  ...
  <clipPath id="glitch-clip"><rect x="${visual.x}" y="${visual.y}" width="${visual.width}" height="${visual.height}" rx="${visual.radius}"/></clipPath>
  ...
  <style>
    ...
    .glitch-error { font-family: 'Courier New', Consolas, monospace; font-size: 10px; letter-spacing: 1px; }
